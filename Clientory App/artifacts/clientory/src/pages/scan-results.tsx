...
function ProviderResult({ r }: { r: PromptWithResults["results"][number] }) {
  ...
  return (
    <div className="p-5 flex flex-col gap-2">
      ...
      {hasResponse ? (
        <div>
          <p className={`text-xs text-foreground/70 leading-relaxed ${expanded ? "" : "line-clamp-2"}`}>
            {r.response}
          </p>
          <button
            onClick={() => setExpanded((v) => !v)}
            className="mt-1.5 flex items-center gap-0.5 text-xs text-primary hover:underline"
          >
            {expanded ? <><ChevronUp className="w-3 h-3" /> Hide response</> : <><ChevronDown className="w-3 h-3" /> View full response</>}
          </button>
        </div>
      ) : (
